...
async function covidCasesByZipcode( zipCode, days = 7, rateOfChange = false ){
  ...
  if( currentDate == lastUpdate ){
    ...
    try {
      let response = await fetch("https://localcoviddata.com/covid19/v1/cases/newYorkTimes?zipCode=" + zipCode + "&daysInPast=7")
  ...
  if( rateOfChange ){
    ...
    return addRateOfChange(["deathCt", "positiveCt"], data).slice(0, arrayLength)
  } else {
    return data
  }
...
async function covidCasesByState( state, days = 31 ){
  ...
  if( currentDate == lastUpdate ){
    ...
    try {
      ...
      let response = await fetch("https://api.covidtracking.com/v1/states/" + state + "/daily.json");
  ...
  return addRateOfChange(["positive", "probableCases", "negative", "pending", "totalTestResults",
                          "hospitalizedCurrently", "hospitalizedCumulative", "inIcuCurrently",
                          "inIcuCumulative", "recovered", "death", "hospitalized"], 
                          data.slice(0, days + 1)).slice(0, days)
