...
export default function LocalInfo(props) {
  ...
  const deathData = cleanData.map((row) => {
    return { x: row.date, y: row.deathCt }
  })
  const positiveData = cleanData.map((row) => {
    return { x: row.date, y: row.positiveCt }
  })
  const positiveChangeData = []
  for (let i = 1; i < positiveData.length; i++) {
    positiveChangeData.push({
      x: positiveData[i].x,
      y: positiveData[i].y - positiveData[i - 1].y,
    })
  }
